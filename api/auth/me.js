...
const GAMES_MAX_BONUS_PER_DAY = 5
...
export default async function handler(
  ...
  if (
    ...
    try {
      ...
      // [investigate-v2] webhook-result
      // نحفظ "شنو صار" بهذا الإعلان (نوعه ونتيجته) داخل نفس السجل، عشان لوحة
      // الأدمن تعرض لكل إعلان سببه. أي فشل هنا ما يأثر على الـ webhook.
      if (logRow && logRow.id) {
        ...
        res.json = (body) => {
          try {
            ...
            const result = {
              type: b.type || (b.success === false ? 'error' : null),
            }
            if (b.gamesBonusLimitReached) result.gamesBonusLimitReached = true
            if (b.starsAdLimitReached) result.starsAdLimitReached = true
            if (b.success === false) result.error = String(b.error || '').slice(0, 120)

            supabase
              .from('adsgram_reward_log')
              .update({ query: { ...safeQuery, _result: result } })
              .eq('id', logRow.id)
              .then(
                () => {},
                () => {}
            ...
            /* تسجيل النتيجة ما لازم يكسر الـ webhook */
          }
  ...
  if (
    ...
    if (
      ...
      // [investigate-v2] backend
      // سجل الإعلانات (آخر 60) + إحصائيات سريعة، مع نوع كل إعلان.
      const { data: adRewardLogRaw, error: adRewardLogError } = await supabase
        .from('adsgram_reward_log')
        .select('*')
        .eq('telegram_id', telegramId)
      ...
      if (adRewardLogError) {
        return res.status(500).json({ success: false, error: adRewardLogError.message })
      }

      const countAds = async (sinceMs, kind) => {
        try {
          let q = supabase
            .from('adsgram_reward_log')
          ...
          if (kind) q = q.eq('query->>kind', kind)
      ...
      const [adsTotal, adsLast10m, adsLastHour, adsLastDay, adsNativeTotal] =
        await Promise.all([
          countAds(0, null),
          countAds(10 * 60 * 1000, null),
          countAds(60 * 60 * 1000, null),
          countAds(24 * 60 * 60 * 1000, null),
          countAds(0, 'native_task'),
        ])

      const adRewardLog = (adRewardLogRaw || []).map((row) => {
        let q = row.query
        if (typeof q === 'string') {
          try { q = JSON.parse(q) } catch (e) { q = {} }
        }
        q = q && typeof q === 'object' ? q : {}
        const { _result, kind, userid, adsgram_reward, ...extra } = q
        return {
          id: row.id,
      ...
      const nativeAdCoins = (adsNativeTotal || 0) * NATIVE_TASK_AD_REWARD_COINS
      ...
      const coinFlow = {
        ...
        nativeAds: adsNativeTotal,
        nativeAdsCoins: nativeAdCoins,
        gifts: giftCoins,
        giftsCount: giftCount,
        other: Math.max(0, coinsEverEarned - tasksCoins - nativeAdCoins - giftCoins),
      ...
      return res.status(200).json({
        ...
        adStats: {
          total: adsTotal,
          last10m: adsLast10m,
          lastHour: adsLastHour,
          lastDay: adsLastDay,
          nativeTotal: adsNativeTotal,
        },
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      if (action === 'native_task_claim') {
        ...
        await logLedger(telegramId, 'native_task_ad', {
          coins: NATIVE_TASK_AD_REWARD_COINS,
          detail: 'إعلان مهمة',
          balanceAfter: Number(claimedRows[0].coin),
        })
