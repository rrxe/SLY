...
const REFERRAL_REWARD_USDT = 0.03
const REFERRAL_REQUIRED_TASKS = 5
const ONLINE_THRESHOLD_MINUTES = 2

const BASE_WITHDRAW_ADS = 10
const EXTRA_ADS_PER_WITHDRAWAL = 5
  ...
  if (
    ...
    try {
      const {
        ...
        .from('players')
        .select(
          'withdrawal_ads_watched, withdrawals_today, withdrawal_count_date'
        )
        .eq(
          'telegram_id',
          telegramId
  ...
  if (
    ...
    if (
      ...
      const {
        ...
        .from('players')
        .select(
          'telegram_id, username, coin, is_banned'
        )
        .order(
          'coin',
          {
