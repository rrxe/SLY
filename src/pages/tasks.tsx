...
function isJoinChannelTask(task: ServerTask) {
  return String(task.task_type || "").toLowerCase() === "join_channel";
}

function isAdLikeTask(task: ServerTask) {
  return isSmartAdTask(task) || isAdsGalaxyTask(task) || isGigaPubTask(task) || isAdsGramTask(task);
}
...
function AdsTaskIcon() {
  return (
    <svg viewBox="0 0 24 24" className="task-svg" aria-hidden="true">
      <rect x="3.5" y="6" width="17" height="12" rx="2.5" fill="none" stroke="currentColor" strokeWidth="1.8" />
      <path d="M8 10.5h5M8 13.5h8" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" />
      <circle cx="17.2" cy="9" r="1.1" fill="currentColor" />
    </svg>
  );
}

function ChannelTaskIcon() {
  return (
    <svg viewBox="0 0 24 24" className="task-svg" aria-hidden="true">
      <path d="M4 12l16-7-6 16-2.5-6.5L4 12z" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinejoin="round" />
    </svg>
  );
}

function GroupAdsIcon() {
  return (
...
export default function Tasks({ onRewardCoins }: Props) {
  ...
  const renderTaskRow = (task: ServerTask, variant: "ads" | "channel" | "other") => {
    const id = String(task.id);
    ...
    const isAdLike = isAdLikeTask(task);
    const claimDelayMs = getClaimDelayMs(task);
    const progress = progressById[id] || { completed: 0, max_completions: Math.max(1, Number(task.max_completions || 1)) };
    const openedAt = openedAtById[id];
    ...
    return (
      <article key={id} className={`task-row variant-${variant} ${claimedAll ? "done" : ""}`}>
        <div className="task-icon">
          {variant === "ads" ? <AdsTaskIcon /> : variant === "channel" ? <ChannelTaskIcon /> : <TaskIcon />}
        </div>
        <div className="task-main">
          ...
          <div className="task-mini-status">
            {claimedAll ? (
              ...
              <span className="gray">{isAdLike ? "Watch the ad to earn coins" : "Open link first"}</span>
            ) : !waitedEnough ? (
        ...
        <div className="task-actions">
          <div className="task-progress-mini">
            <strong>{progress.completed}</strong> <em>/ {progress.max_completions}</em>
          </div>
          <button type="button" className="task-btn join" onClick={() => handleOpenTask(task)} disabled={opening || claimedAll}>
            {opening ? (isAdLike ? "Loading ad..." : "Opening...") : (isAdLike ? "Watch Ad" : "Open")}
          </button>
        </div>
      </article>
    );
  ...
  const adsTasks = serverTasks.filter((t) => isAdLikeTask(t));
  const channelTasks = serverTasks.filter((t) => isJoinChannelTask(t));
  const otherTasks = serverTasks.filter((t) => !isAdLikeTask(t) && !isJoinChannelTask(t));
          ...
          <>
            {adsTasks.length > 0 && (
              <div className="task-group">
                <div className="task-group-title ads">
                  <span className="icon-badge"><GroupAdsIcon /></span>
                  <span className="label">Ads Tasks</span>
                  <span className="count">{adsTasks.length}</span>
                </div>
                {adsTasks.map((t) => renderTaskRow(t, "ads"))}
              </div>
            )}

            {channelTasks.length > 0 && (
              <div className="task-group">
                <div className="task-group-title channel">
                  <span className="icon-badge"><GroupChannelIcon /></span>
                  <span className="label">Channel Tasks</span>
                  <span className="count">{channelTasks.length}</span>
                </div>
                {channelTasks.map((t) => renderTaskRow(t, "channel"))}
              </div>
            )}

            {otherTasks.length > 0 && (
              <div className="task-group">
                <div className="task-group-title other">
                  <span className="icon-badge"><GroupOtherIcon /></span>
                  <span className="label">Other Tasks</span>
                  <span className="count">{otherTasks.length}</span>
                </div>
                {otherTasks.map((t) => renderTaskRow(t, "other"))}
              </div>
            )}
          </>
        )}
