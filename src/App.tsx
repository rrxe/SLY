...
const ADSGRAM_STARS_BLOCK_ID = "int-46338";
...
export default function App() {
  ...
  // نتجاهله - لأنه أكيد قرا القيمة قبل ما الإعلان ينحسب بالسيرفر،
  // وبيرجع يفضّي (overwrite) العداد الصحيح بقيمة قديمة لو وصل متأخر.
  ...
  const handleWatchStarsAd = async () => {
    ...
    const cooldownRemainingMs = starsAdCooldownUntilRef.current - Date.now();
    if (cooldownRemainingMs > 0) {
      setStarsAdToast(
        `Please wait ${Math.ceil(cooldownRemainingMs / 1000)}s before watching another ad.`
      );
      return;
    }
